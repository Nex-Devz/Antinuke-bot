export async function handlePermissionOverwriteCreate(event, context) {
  ...
  if (detectedDangerous.length > 0) {
    ...
    if (risk >= 70) {
      await event.overwrite.delete('AntiN8: Dangerous permission overwrite detected').catch(() => null);
    ...
    if (risk >= 70) {
      ...
      if (snapshot?.overwrites) {
        ...
        if (originalOverwrite) {
          await event.overwrite.update({ allow: originalOverwrite.allow, deny: originalOverwrite.deny }, 'AntiN8: Reverting dangerous permission change').catch(() => null);
        } else {
          await event.overwrite.delete('AntiN8: Removing dangerous permission overwrite').catch(() => null);
        }
      }
